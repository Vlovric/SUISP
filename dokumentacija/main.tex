...
Softver će biti implementiran koristeći programski jezik Python. Bit će klijentski (bez poslužitelja), odnosno izvršavat će se na jednom računalu za jednog korisnika.
Softver je zamišljen da funkcionira na operacijskim sustavima Linux i Windows i da se može koristiti bez ikakvih ovisnosti, odnosno da korisnik ne mora sam instalirati Python, nego je dovoljno samo aplikaciju.

Aplikacija će imati korisničko sučelje, odnosno GUI.
Za grafičko sučelje će se koristiti Python modul \textit{PySide6}, koji je "službeni Python modul za \textit{Qt for Python project}, koji pruža pristup kompletnom \textit{Qt} 6.0+ okviru" \cite{pyside6}.
Dostupan je na sljedećoj poveznici: \url{https://pypi.org/project/PySide6/}.

Za kriptografske funkcionalnosti koristit će se već poznate Python biblioteke, kao što je \textit{cryptography} modul.
\textit{Cryptography} modul je dostupan na sljedećoj poveznici: \url{https://cryptography.io/en/latest/}.



\chapter{Implementacija}

Implementirana je kompletna aplikacija koristeći tehnologije navedene u poglavlju "Tehnologije za implementaciju" i implementirana sve spomenute funkcionalne i nefunkcionalne zahtjeve.
Može se pokretati na Linux i Windows operacijskim sustavima, bez potrebnog instaliranja drugih aplikacija i modula.
Sljedeća poglavlja opisuju kako su implementirani glavni funkcionalni zahtjevi.

\section{Prijava, registracija i odjava}

Implementacija aplikacije započinje izradom registracije odnosno postavljanjem inicijalne lozinke u sustav.
Aplikacija prepoznaje ako ne postoji niti jedan korisnik u sustavu te upućuje korisnika na zaslon registracije. Ako korisnik već postoji u bazi, aplikacija korisnika upućuje na zaslon prijave. Važno je napomenuti da se do registracije ne može doći niti jednim drugim načinom osim ako u bazi ne postoji korisnik, stoga se aplikacija može korisititi isklučivo od strane jednog korisnika koji je ujedno i vlasnik profila na računalu. 

\begin{figure}[h]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{slike/prijava_suisp.png} 
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{slike/registracija_suisp.png}
    \end{subfigure}
    \caption{Zasloni prijave i registracije}
\end{figure}
...
Aplikacija provjerava i minimalnu entropiju lozinke koja ne smije niti manja od 80 bitova. Ukoliko lozinka ne zadovoljava navedene uvjete, korisnik dobiva povratnu informaciju o tome što treba popraviti. 

Lozinka se, nakon upisa, hashira koristeći \textbf{PBKDF2 algoritam} s \textbf{SHA-256} funkcijom sažetka i \textbf{600000 iteracija}. Algoritam kao parametre prima \textit{Master key} (objašnjeno u nastavku) i lozinku u obličnom tekstu kao sol.
...
\subsection{Dodatni zaštitni mehanizmi prijave i odjave}

Kako bi se spriječili \textit{bruteforce} napadi na lozinku, implementirani je \textit{lockout} mehanizam. Nakon 3 nesupjela pokušaja prijave, vrijeme čekanja za sljedeća 3 pokušaja povećava za minutu (npr. 3 neuspjela = 60s, 6 neuspjelih = 120s...). Važno je napomenuti da nema gornje granice za broj pokušaja. Kako bi se spriječilo ponovno postavljanje i zaobilaženje vremena čekanja ako se korisnik ugasi aplikaciju ili računalo, vrijeme posljednjeg neuspjelog pokušaja prijave sprema se u bazu podataka i koristi se za izračun vremena čekanja prilikom svakog novog pokušaja prijave.

\begin{figure}[h]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{slike/odbrojavanje_60.png} 
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{slike/odbrojavanje_120.png}
    \end{subfigure}
...
\includegraphics[width=1\linewidth]{slike/automatska_odjava.png}
\caption{Zaslon automatske odjave nakon neaktivnosti}
\end{figure}

Također je osigurana samo jedna instanca aplikacije kao zaštita protiv \textit{bruteforce} napada, ali i kako bi se osigurao ispravan rad sustava u slučaju da korisnik namjerno ili slučajno pokuša otvoriti više instanci aplikacije. Jedna instanca aplikacije je osigurana komunikacijom putem lokalnih utičnica (engl. \textit{local socket}) na način gdje inicijalno otvorena instanca aplikacije pokreće \textit{server} koristeći identifikator aplikacije, i svaka instanca aplikacije koja se pokuša pokrenuti nakon prve će provjeriti postojanje \textit{servera} putem lokalne utičnice, i ako \textit{server} postoji, ugasiti svoju instancu i poslati originalnoj instanci poruku kako bi se prebacila u fokus, npr. stavila u prvi plan ili maknula iz minimizacije.\newline
Kao i ostatak implementacije, i ovaj mehanizam je u potpunosti kompatibilan sa svim operacijskim sustavima zbog korištenja \textit{QLocalSocket} i \textit{QLocalServer} biblioteka koje koriste \textit{unix domain socket} na Linux/macOS sustavima i \textit{imenovane cjevovode} na Windows sustavima.
...
Prije kriptiranja privatnog ključa generiraju se dva međuključa pomoću \textbf{PBKDF2 algoritma}. U prvoj iteraciji, generira se \textit{Master key} (dalje u teksu MK) sa velikim brojem iteracija (600000) nakon kojega se generira \textit{Password Derived Key} (dalje u tekstu PDK) s manjim brojem iteracija (1) koji se koristi za kriptiranje privatnog ključa. PDK ključ se NE sprema u bazu nego ostaje samo u memoriji tijekom trajanja sesije.
...
\subsection{Procedura sigurnog zatvaranja aplikacije i oporavka od pogreške}

S obzirom da korisnik ručno otključava i zaključava datoteke prema potrebi, nužno je osigurati dva slučaja: situacija gdje korisnik izađe iz aplikacije dok ima otključane datoteke i situacija gdje se aplikacija prisilno ugasi kroz ubijanje procesa ili naglog prestanka rada uređaja dok korisnik ima otključane datoteke. \newline
\textbf{U slučaju da korisnik izađe iz aplikacije bez da je zaključao sve datoteke}, neovisno je li se izlazak izvršio putem gumba za odjavu, pritiskom "X" gumba za izlaz na prozoru aplikacije, prečacem ili slično, sustav će u pozadini zaključati sve otključane datoteke korisnika istim algoritmom zaključavanja koji se odvija i kada korisnik ručno zaključa datoteku. \newline
\textbf{U slučaju da sustav prisilno prestane s radom, a ima otključane datoteke}, prilikom sljedeće prijave u sustav se sigurno brišu sve otključane datoteke jer se smatraju komprimiranima. Korisniku ostaje dostupna njegova prošla zaključana verzija datoteke u trezoru.

\section{Sigurnosna politika i rotacija ključeva}

Sustav omogućava definiranje sigurnosne politike, tj. kontrolu nad većinom parametara koji se koriste u radu sustava. Parametri sigurnosne politike se mogu podijeliti u četiri glavne vrste:
\begin{itemize}
    \item \textbf{Parametri za algoritme} - ovi parametri se koriste u algoritmima generiranja ključeva, stvaranja sažetaka, kriptiranja i dekriptiranja i slično. Neki od njih su \textit{duljina ključa}, \textit{duljina inicijalizacijskog vektora}, \textit{broj iteracija za PBKDF2 algoritam} i \textit{algoritam sažimanja u PBKDF2}.
    \item \textbf{Parametri za lozinku} - ovi se parametri koriste za minimalne karakteristike lozinke korisnika kako bi se osigurala željena minimalna razina entropije i kompleksnosti. Neki od njih su \textit{duljina lozinke}, \textit{minimalna entropija}, \textit{nužnost brojeva, znakova i ostaloga} i \textit{mogući posebni znakovi}
    \item \textbf{Parametri za neaktivnost} - ovi parametri određuju broj minuta prije nego sesija korisnika završi zbog neaktivnosti
    \item \textbf{Interval dana između rotacije ključeva} - ovaj parametar određuje maksimalan interval dana između rotacije ključeva, nakon čega se vrši automatska rotacija
\end{itemize}

Rotacija ključeva se može odviti u dva slučaja: automatski od strane sustava ili ručno od strane korisnika. Nakon prijave korisnika, sustav provjerava zadnji datum rotacije i uspoređuje ga s propisanim intervalom u sigurnosnoj politici te izvršava rotaciju ukoliko je potrebno. Pri izvršavanju rotacije, korisniku se prikazuje napredak rotacije kroz novi prozor koji blokira korisničko sučelje kako korisnik ne bi mogao obaviti nijednu radnju ili izaći iz aplikacije dok se rotacija obavlja s obzirom da je to osjetljiva radnja. U pozadini se rotacija odvija na bazi transakcije, tj. tek nakon što su svi ključevi rotirani i sve je obavljeno se promjene spremaju u bazu. Ovo je napravljeno kako prisilno gašenje tijekom rotacije ne bi oštetilo funkcionalnost sustava zbog djelomične rotacije. Pojednostavljeni algoritam koji se odvija je sljedeći:
\begin{enumerate}
    \item Generira se novi par ključeva.
    \item Uzima se DEK svake datoteke.
    \item DEK se dekriptira postojećim privatnim ključem i kriptira novim KEK-om (javnim ključem).
    \item Nakon kriptiranja svih DEK-ova kriptira se novi privatni ključ.
    \item Spremaju se svi rotirani DEK-ovi, novi par ključeva i ažurira datum zadnje rotacije.
    \item Radnja se bilježi u revizijski trag.
\end{enumerate}

Sljedeća slika prikazuje prozor koji potvrđuje uspješnu rotaciju ključeva:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{slike/rotacija_popup.png}
    \caption{Uspješna rotacija ključeva}
\end{figure}
...
\section{Otključavanje i zaključavanje datoteke}

Nakon inicijalnog prijenosa datoteke u trezor, ona se može otključavati i zaključavati. Način otključavanja u ovom sustavu je takav da se u memoriji dekriptira sadržaj datoteke koja se otključava i da se ona pohrani u privremeni (engl. \textit{temporary}) direktorij na operacijskom sustavu te se automatski pokreće proces tj. aplikacija koja je zadužena za tu vrstu datoteku na uređaju. Nakon što je korisnik gotov s pregledavanjem ili uređivanjem datoteke, mora ju ponovno zaključati. Pojednostavljeni algoritam za otključavanje je sljedeći:
\begin{enumerate}
    \item Privatni ključ korisnika se dekriptira koristeći PDK.
    \item DEK datoteke se dekriptira koristeći privatni ključ.
    \item Datoteka se dekriptira koristeći DEK.
    \item Provjerava se trenutni sažetak datoteke s pohranjenim sažetkom u prethodnom zaključavanju.
    \item Datoteka se sprema u privremeni direktorij.
    \item Otključavanje se bilježi u revizijski trag.
    \item Pokreće se aplikacija zadužena za tu vrstu datoteke.
\end{enumerate}

Prilikom zaključavanja se koristi vrlo sličan algoritam kao i kod inicijalnog prijenosa, ali se nakon pohrane novo-kriptirane datoteke sigurnosno briše \textit{plaintext} datoteka iz privremenog direktorija jer više nije potrebna. Pojednostavljeni algoritam za zaključavanje je sljedeći:
\begin{enumerate}
    \item Datoteka se učitava iz privremenog direktorija.
    \item Stvara se sažetak datoteke i pohranjuje.
    \item DEK datoteke se dekriptira koristeći privatni ključ.
    \item Datoteka se kriptira koristeći DEK.
    \item Zaključavanje se bilježi u revizijski trag.
    \item Kriptirana datoteka se sprema u direktorij trezora.
    \item Sigurnosno se briše \textit{plaintext} datoteka iz privremenog direktorija.
\end{enumerate}

Sljedeća slika prikazuje zaslon nakon što je datoteka otključana:

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{slike/otkljucana_datoteka.png}
    \caption{Otključana datoteka s gumbom za zaključavanje}
\end{figure}


\section{Brisanje datoteke}

Pritiskom gumba za brisanje na odabranoj datoteci, poziva se funkcija koja prvo traži postoji li odabrana kriptirana datoteka na korisnikovom računalu. Ako postoji, datoteka se prvo otvara, prepisuje tako što se upišu nulti bajtovi preko cijele datoteke, zatim se zatvara i briše iz datotečnog sustava. Nakon brisanja datoteke na računalu, briše se i zapis o datoteci iz baze podataka. Sve ove radnje bilježe se u revizijski trag. 

Implementirana je zaštita od pogrešnog brisanja datoteka tako što se pritiskom na gumb otvara dijalog prozor koji traži potvrdu brisanja datoteke. Kao potvrda za brisanje, korisnik mora unjeti naziv datoteke koju želi izbrisati. Ako uneseni naziv odgovara nazivu datoteke koja se briše, datoteka se briše, inače se ispisuje greška.

\begin{figure}[h]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{slike/dijalog_brisanje_datoteke.png} 
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{slike/dijalog_brisanje_greska.png}
    \end{subfigure}
...
\begin{enumerate}
    ...
    \item Dohvaća sve zapise revizijskog traga iz baze podataka. \textbf{U slučaju da lanac sažetaka (engl. \textit{hash chain}) zapisa ne odgovara, u zapisima će biti naznačeno.}
...
Digitalni sigurnosni trezori nisu uobičajeni sustavi za pohranu podataka. U njima datoteke, osim što su pohranjene, su i zaštićene mnoštvom složenih kriptografskih algoritama i sigurnosnim mehanizmima. Takvi sustavi su uglavnom implementirani pomoću Web tehnologija i nalaze se na internetu kako bi korisnicima omogućili kolaboraciju, dijelenje i pristup podacima s bilo koje lokacije. Mana takvih sustava je upravo to da su na mreži te su puno više izloženi prijetnjama i napadima. Ovakvi trezori su već odavno u upotrebi u velikim organizacijama koje rukuju osjetljivim podacima kao što su financijski dokumenti, medicinski zapisi, pravni dokumenti itd. Dostupni su i komercijalni proizvodi za osobnu upotrebu iako su poprilično skupi i nisu baš toliko popularni kod običnih korisnika.

S druge strane, postoje klijentski sigurnosni trezori, odnosno aplikacije koje se mogu instalirati na računalo i zaštititi datoteke korisnika. Takvi sustavi sigurniji su u pogledu mrežnih napada i prijetnji jer nemaju taj vektor napada. Takvi sustavi su dosta rijetki i nisu toliko razvijeni jer nema velikog broja korisnika za njima. Često takvi alati i nisu javno dostupni ili su pak razvijeni za interne potrebe velikih organizacija.

U radu smo pokušali razviti jedan takav sustav koji bi zadovoljio potrebe malih korisnika. Najveća prednost ovog rada je taj što se datoteke u sustavu uništavaju i čiste tako da ako i dođe do napada na korisnikovo računalo, datoteke neće biti dostupne. Poštivanjem standarda ISO 27001, pokušali smo dovesti sustav do visoke razine sigurnosti. Služio nam je kao vodič za implementaciju mehanizama unutar aplikacije. Bez njega, teško bi znali od kuda početi i što sve zapravo treba implementirati da bi sustav bio dovoljno siguran. Trenutna implementacija sadrži dosad najbolje kriprografske algoritme koji ne bi trebali biti probijeni neko duže vrijeme. Sigurnost sustava također ovisi i o korisniku koji ga koristi i o tome kako će on postaviti parametre svoje sigurnosne politike. Sustav nije savršen i uvjek postoji mogućnost poboljšanja, ali za sada je ovo odličan početak. 
